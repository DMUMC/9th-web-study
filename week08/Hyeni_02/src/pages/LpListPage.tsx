...
export const LpListPage = () => {
  ...
  useEffect(() => {
    console.log("Throttled Scroll Check", throttledScrollY);

    if (
      window.innerHeight + throttledScrollY >= document.body.offsetHeight - 100 &&
      hasNextPage &&
      !isFetchingNextPage
    ) {
      console.log("바닥 도달, 로딩 시작");
      fetchNextPage();
    }
          ...
          <div className="relative w-full">
             ...
             <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <div className="relative">
                    ...
                    {isDropdownOpen && (
                        <div className="absolute right-0 mt-2 w-32 bg-[#2c2c2c] border border-[#444] rounded-xl shadow-2xl overflow-hidden flex flex-col z-30">
                            <button onClick={() => { setSearchType('title'); setIsDropdownOpen(false); }} className={`px-4 py-3 text-left text-sm hover:bg-[#2c1ee9] hover:text-white transition-colors ${searchType === 'title' ? 'text-[#2c1ee9] font-bold' : 'text-gray-300'}`}>제목</button>
                            <button onClick={() => { setSearchType('tag'); setIsDropdownOpen(false); }} className={`px-4 py-3 text-left text-sm hover:bg-[#2c1ee9] hover:text-white transition-colors ${searchType === 'tag' ? 'text-[#2c1ee9] font-bold' : 'text-gray-300'}`}>태그</button>
                        </div>
                    )}
                </div>
             </div>
