    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-6 mb-8">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-extrabold gradient-text">찾기</h1>
        </div>
              ...
              <button
                ...
                className={`rounded-full px-5 py-2.5 text-sm font-semibold transition-all ${
                  isActive
                    ...
                    : "bg-white text-gray-600 shadow-soft hover:shadow-md hover:scale-105"
                }`}
              >
                {option.label}
              </button>
          ...
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {lpList.map((lp) => {
              ...
              return (
                <article
                  key={lp.id}
                  className="group relative cursor-pointer overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 shadow-xl card-hover"
                  onClick={() => {
                    if (lp?.id) {
                      navigate(`/lp/${lp.id}`);
                    }
