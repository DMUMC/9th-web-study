...
interface MovieDetailHeader {
	movie: MovieDetails;
}

export const MovieDetailHeader = ({ movie }: MovieDetailHeader) => {
	return (
		<div
					...
					<div className='flex flex-col justify-end text-white pb-4'>
						...
						<div className='flex gap-2 mb-4'>
							{movie.genres.map((genre) => (
								<span key={genre.id} className='px-3 py-1 bg-emerald-600 rounded-full text-sm'>
									{genre.name}
								</span>
							))}
						</div>
