...
import type { CreditResponseT, CreditT } from "../types/credit"
    ...
    useEffect(() => {
        const getMovieDetail = async () => {
            try {
                setIsLoading(true)
                setError(null)
                const res = await api.get<MovieDetailT>(`/movie/${movieId}?language=ko-KR`)
                setMovie(res.data)

                const creditRes = await api.get<CreditResponseT>(`/movie/${movieId}/credits?language=ko-KR`)
